...
func main() {
  ...
  //Create root certificate template
	rootTemplate := x509.Certificate{
		...
		Subject: pkix.Name{
			Organization: []string{"Vnpay Corporation"},
			CommonName:   "Root CA",
		},
		NotBefore:             notBefore,
		NotAfter:              notAfter,
...
func keyToFile(filename string, key *ecdsa.PrivateKey) {
		...
		file, err := os.Create("cert/"+filename)
		if err != nil {
			panic(err)
		}
		defer file.Close()
...
func certToFile(filename string, derBytes []byte) {
		os.MkdirAll("cert", os.ModePerm);
		certOut, err := os.Create("cert/"+filename)
		if err != nil {
...
func debugCertToFile(filename string, derBytes []byte) {
	os.MkdirAll("debug", os.ModePerm);
	cmd := exec.Command("openssl", "x509", "-text", "-inform", "DER")

	file, err := os.Create("debug/"+filename)
	if err != nil {
		panic(err)
	}
	defer file.Close()
