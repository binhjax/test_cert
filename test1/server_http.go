...
func main() {
	...
	certsPath := flag.String("letsencrypt-path", "cert", "path at which an 'xyz.example.com' containing 'fullchain.pem' and 'privkey.pem' can be found")
	defaultHost := flag.String("default-hostname", "localhost", "the default folder to find certificates to use when no matches are found")
	...
	*/
	...
	privkeyPath := filepath.Join(*certsPath, "leaf.key")
	certPath := filepath.Join(*certsPath, "leaf.pem")
